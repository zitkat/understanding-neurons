...
# %% [markdown]
# # Distribution of critical units in MobileNetv3 RW
...
# %%
p_df = pickle.load(open((data_path / f"{model_name}_{stage}" / "crit_df").with_suffix(".pickle"),'rb'))

# %%
# p_df = add_criticality_data(p_df, from_json=Path("../data/criticals/mobilenetv3_rw/statistics_dict.json"))
# pickle.dump(p_df, open((data_path / f"{model_name}_{stage}" / "crit_df").with_suffix(".pickle"),'wb'))
...
fig, axs = plot_parametrized_var(p_df, x_var="x", y_var="y",
                                 row_var="block",
                                 column_var="branch", 
                                 color_var="mountain_bike", color_lab="Crit",
#                                  mk_var="conv",
